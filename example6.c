...
int main() {
    ...
    if (unitChoice == 2) {
        pricePerBarrelCAD /= GALLON_TO_LITER;
    } else if (unitChoice == 3) {
        pricePerBarrelCAD /= BARREL_TO_GALLON;
    }

    // Calculate equivalent prices
    double pricePerGallonUSD = pricePerBarrelCAD * USD_TO_CAD / BARREL_TO_GALLON;
    double pricePerLiterUSD = pricePerGallonUSD * GALLON_TO_LITER;
    double pricePerLiterCAD = pricePerLiterUSD * USD_TO_CAD;

    // Display the results
    printf("--- Unit --- USD ----- CAD ---\n");
    printf("Gallon: $ %.2lf $ %.2lf\n", pricePerGallonUSD, pricePerBarrelCAD * USD_TO_CAD / BARREL_TO_GALLON);
